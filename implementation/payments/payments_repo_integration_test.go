...
func Test_Create_Returns_ValidationError_When_PaymentAttributes_SponsorParty_Invalid(t *testing.T) {
	DB := testhelpers.DBConnection(t, &schema.Payment{})
	incomingPayment := schema.ValidPayment()
	incomingPayment.Attributes.SponsorParty.AccountNumber = ""
	createdPayment, err := Create(DB, incomingPayment)
	assert.NotNil(t, err)
	assert.Nil(t, createdPayment)
	_, ok := err.(*ValidationError)
...
func Test_SetAll_Returns_ValidationError_If_Any_PaymentAttributes_SponsorParty_Are_Invalid(t *testing.T) {
	DB := testhelpers.DBConnection(t, &schema.Payment{})
	badEgg := schema.ValidPayment()
	badEgg.Attributes.SponsorParty.BankID = ""
	allPayments := []schema.Payment{
		*schema.ValidPayment(),
		*schema.ValidPayment(),
		*badEgg,
...
func Test_Update_Returns_Updated_Payment_When_Payment_Exists_And_Valid_Update(t *testing.T) {
	...
	incomingPayment.Attributes.SponsorParty.AccountNumber = "33333"
...
func Test_Update_Returns_ValidationError_When_PaymentAttributes_SponsorParty_Update_Invalid(t *testing.T) {
	DB := testhelpers.DBConnection(t, &schema.Payment{})
	payment := schema.ValidPayment()
